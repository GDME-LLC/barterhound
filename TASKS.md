...
- [x] Create folder structure for `src/app`, `src/lib`, `src/types`, and a reserved `src/components`
- [x] Add `package.json`, `tsconfig.json`, `tailwind.config.ts`, `postcss.config.mjs`, `next.config.mjs`
- [x] Add `.gitignore` and `.env.example`
- [x] Scaffold all app routes with placeholder pages
- [x] Add shared TypeScript types (`src/types/index.ts`)
- [x] Add Supabase helper stubs (`src/lib/supabase/client.ts`, `server.ts`)
- [x] Add planning assets for later backend phases (`supabase/schema.sql`, domain types, product docs)
- [x] Write `README.md` with setup instructions and architecture overview
- [x] Write `SPEC.md` - product specification
